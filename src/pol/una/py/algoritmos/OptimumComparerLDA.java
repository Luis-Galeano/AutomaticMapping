 ...
 * @author Luis Galeano
 */
public class OptimumComparerLDA {
    public static void main(String[] args) {
        try {
            ...
            SentencePreprocessor preprocessor = new SentencePreprocessor(SentencePreprocessor.TokenizerType.STANFORD,  SentencePreprocessor.TaggerType.OPENNLP_PERCEPTRON, SentencePreprocessor.StemmerType.WORDNET_SIMPLE, SentencePreprocessor.ParserType.NONE);
                        ...
                        for (boolean baseForm : Util.booleans){
                            for (WordWeightType wordWeightType : PairwiseComparer.WordWeightType.values()){
                                for(NormalizeType normalizeType: PairwiseComparer.NormalizeType.values()){
                                    comparer.setOptimumComparerLDATasa(new OptimumComparer(new LDAWordMetric("LDA-MODEL-TASA-LEMMATIZED-TOPIC300"), wSim, baseForm, wordWeightType, normalizeType));
                                    try{
                                        ...
                                        String a = "optimumComparerLDA";
                                        String p = wSim+" "+baseForm+" "+wordWeightType.toString()+" "+normalizeType.toString();
